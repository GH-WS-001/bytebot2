...
export class AgentProcessor {
  ...
  private async runIteration(taskId: string): Promise<void> {
    ...
    try {
      ...
      // 输出请求内容到日志
      // this.logger.debug('=== Agent Request to LLM ===');
      // this.logger.debug(`System Prompt: ${AGENT_SYSTEM_PROMPT.substring(0, 200)}...`);
      // messages.forEach((msg, index) => {
      //   this.logger.debug(`Message ${index + 1} [${msg.role}]:`);
      //   (msg.content as MessageContentBlock[]).forEach((block, blockIndex) => {
      //     if (block.type === MessageContentType.Text) {
      //       this.logger.debug(`  Text Block ${blockIndex + 1}: ${block.text.substring(0, 200)}${block.text.length > 200 ? '...' : ''}`);
      //     } else if (block.type === MessageContentType.ToolUse) {
      //       this.logger.debug(`  Tool Use Block ${blockIndex + 1}: ${block.name}`);
      //       this.logger.debug(`    Input: ${JSON.stringify(block.input).substring(0, 200)}${JSON.stringify(block.input).length > 200 ? '...' : ''}`);
      //     } else if (block.type === MessageContentType.ToolResult) {
      //       this.logger.debug(`  Tool Result Block ${blockIndex + 1}: ${block.tool_use_id}`);
      //       this.logger.debug(`    Content: ${JSON.stringify(block.content).substring(0, 200)}${JSON.stringify(block.content).length > 200 ? '...' : ''}`);
      //     }
      //   });
      // });
      // this.logger.debug('=== End Agent Request ===');
      ...
      // 输出响应内容到日志
      // this.logger.debug('=== Agent Response from LLM ===');
      // messageContentBlocks.forEach((block, index) => {
      //   if (block.type === MessageContentType.Text) {
      //     this.logger.debug(`Text Block ${index + 1}: ${block.text.substring(0, 200)}${block.text.length > 200 ? '...' : ''}`);
      //   } else if (block.type === MessageContentType.ToolUse) {
      //     this.logger.debug(`Tool Use Block ${index + 1}: ${block.name}`);
      //     this.logger.debug(`  Input: ${JSON.stringify(block.input).substring(0, 200)}${JSON.stringify(block.input).length > 200 ? '...' : ''}`);
      //   }
      // });
      // this.logger.debug('=== End Agent Response ===');
